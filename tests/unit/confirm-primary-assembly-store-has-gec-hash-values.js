...
class AssemblyStore {
  ...
  is_first_data_offset_correct(debug) {
    // "lec" is the count of filenames in the primary store.
    // ("gec" - "lec") is the count of filenames in each architecture-specific store. These filenames are common to all such stores, though each filename corresponds to different data in each store.

    const assert_data_offset_with_lec_hashes = 20 + (this.hdr_lec * 24) + (this.hdr_lec * 40);
    const assert_data_offset_with_gec_hashes = 20 + (this.hdr_lec * 24) + (this.hdr_gec * 40);
    ...
    if (debug) {
      console.log('assert data offset with "lec" hashes:', assert_data_offset_with_lec_hashes, 'bytes')
      console.log('assert data offset with "gec" hashes:', assert_data_offset_with_gec_hashes, 'bytes')
      console.log('actual data offset:', actual_data_offset, 'bytes')
    }

    return (assert_data_offset_with_gec_hashes === actual_data_offset)
  }
}
...
function unpack_store(args) {
  ...
  const parser = yargs(args)
    .usage('Usage: node confirm-primary-assembly-store-has-lec-hash-values.js [options]')
    .option('blob', {
      alias: 'b',
      type: 'string',
